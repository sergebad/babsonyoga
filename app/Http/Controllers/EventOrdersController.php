...
class EventOrdersController extends MyBaseController
{
    ...
    {
        $allowed_sorts = ['first_name', 'email', 'order_reference', 'order_status_id', 'created_at', 'notes'];

        $searchQuery = $request->get('q');
        $sort_by = (in_array($request->get('sort_by'), $allowed_sorts) ? 'orders.' . $request->get('sort_by') : 'orders.created_at');
        ...
        if ($searchQuery) {
            ...
            $orders = $event->orders()
                ...
                ->where(function ($query) use ($searchQuery) {
                    $query->where('orders.order_reference', 'like', $searchQuery . '%')
                        ->orWhere('orders.first_name', 'like', $searchQuery . '%')
                        ->orWhere('orders.email', 'like', $searchQuery . '%')
                        ->orWhere('orders.last_name', 'like', $searchQuery . '%');
                })
            ...
            $orders = $event->orders()
                ->join('attendees', 'attendees.order_id', '=', 'orders.id')
                ->where('attendees.is_cancelled', 0)
                ->orderBy($sort_by, $sort_order)->paginate();
        }
